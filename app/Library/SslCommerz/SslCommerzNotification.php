...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function orderValidate($post_data, $trx_id = '', $amount = 0, $currency = "IDR")
    {
...
//                    }
    ...
    {
        $this->data['total_amount'] = $info['total_amount']; // decimal (10,2)	Mandatory - The amount which will process by SSLCommerz. It shall be decimal value (10,2). Example : 55.40. The transaction amount must be from 10.00 IDR to 500000.00 IDR
        $this->data['currency'] = $info['currency']; // string (3)	Mandatory - The currency type must be mentioned. It shall be three characters. Example : IDR, USD, EUR, SGD, INR, MYR, etc. If the transaction currency is not IDR, then it will be converted to IDR based on the current convert rate. Example : 1 USD = 82.22 IDR.
